...
int main()
{
  ...
  norm2u3(v, n1, n2, n3, &rnm2, &rnmu, nx[lt], ny[lt], nz[lt], p);
                 ...
                 double a[4], double c[4], int n1, int n2, int n3, FILE *p)
{
  ...
  psinv(&r[ir[k]], &u[ir[k]], m1[k], m2[k], m3[k], c, k, p);
  ...
  for (k = lb+1; k <= lt-1; k++) {
    ...
    //---------------------------------------------------------------------
    zero3(&u[ir[k]], m1[k], m2[k], m3[k]);
    interp(&u[ir[j]], m1[j], m2[j], m3[j], &u[ir[k]], m1[k], m2[k], m3[k], k, p);

    //---------------------------------------------------------------------
    // compute residual for level k
    //---------------------------------------------------------------------
    resid(&u[ir[k]], &r[ir[k]], &r[ir[k]], m1[k], m2[k], m3[k], a, k, p);
  ...
  j = lt - 1;
  k = lt;
  interp(&u[ir[j]], m1[j], m2[j], m3[j], u, n1, n2, n3, k, p);
  resid(u, v, r, n1, n2, n3, a, k, p);
  psinv(r, u, n1, n2, n3, c, k, p);
}
                  ...
                  void *os, int m1j, int m2j, int m3j, int k, FILE *p)
{
  ...
  if (debug_vec[0] >= 1) {
    rep_nrm(s, m1j, m2j, m3j, "   rprj3", k-1, p);
  }
                   ...
                   void *ou, int n1, int n2, int n3, int k, FILE *p)
{
  ...
  if (debug_vec[0] >= 1) {
    rep_nrm(z, mm1, mm2, mm3, "z: inter", k-1, p);
    rep_nrm(u, n1, n2, n3, "u: inter", k, p);
  }
...
static void rep_nrm(void *u, int n1, int n2, int n3, char *title, int kk, FILE *p)
{
  ...
  norm2u3(u, n1, n2, n3, &rnm2, &rnmu, nx[kk], ny[kk], nz[kk], p);
